...
public class TechManager {
    ...
    private final List<List<ETech>> techCondition = Arrays.asList(
            ...
            /*Conditions for T2*/   Arrays.asList(ETech.T1),
    ...
    // ALternative "canDevelop" with ETech list.
    public boolean canDevelop2(ETech tech) {
    ...
    public void develop2(ETech tech) {
        if (canDevelop2(tech)) {
            techList[tech.ordinal()] = true;
            developedTech.add(tech); // check if this will be used
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < t.developedTech.size(); i++) {
            System.out.println(t.developedTech.get(i));
        }
        for (int i = 0; i < ETech.values().length; i++) {
